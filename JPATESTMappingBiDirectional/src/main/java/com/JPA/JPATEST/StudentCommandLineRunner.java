...
import com.JPA.JPATEST.dao.InstructorDetailRepository;
...
public class StudentCommandLineRunner implements CommandLineRunner{
	...
	@Autowired
	InstructorDetailRepository detRepository;
...
//		deleteInstructor(id);
		...
		// find instructor detail data
		InstructorDetail findInstructorDetail = findInstructorDetail(id);
		System.out.println("Instructor Details data: "+findInstructorDetail);
		// from instructor detail data get the Instructor data
		System.out.println("Instructor data from Instructor Details : "+findInstructorDetail.getInstructor());
	...
	{
		// fetch from DB
		Optional<InstructorDetail> optional = detRepository.findById(id);
		if(optional.isEmpty())
		{
			throw new RuntimeException("Data not present for id: "+id);
		}
